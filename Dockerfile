...
RUN a2enmod rewrite
RUN wget https://wordpress.org/wordpress-4.6.1.tar.gz
RUN rm -rf /var/www/html && tar zxf wordpress-4.6.1.tar.gz && mv wordpress /var/www/html && cd /var/www/ && genconfig.sh
RUN rm -f wordpress-4.6.1.tar.gz
